...
function ensureGeocodeProximity(): Promise<void> {
  ...
  geocodeProximityPromise = new Promise((resolve) => {
    if (typeof navigator === "undefined" || !navigator.geolocation) {
      console.error(
        "[TripPhotoUploadPanel] geocoding proximity: navigator.geolocation is not available",
      )
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (Number.isFinite(longitude) && Number.isFinite(latitude)) {
          ...
          console.error(
            "[TripPhotoUploadPanel] geocoding proximity: invalid coordinates from getCurrentPosition",
            { longitude, latitude },
          )
          geocodeProximityState = { kind: "none" }
        }
      ...
      (err: GeolocationPositionError) => {
        console.error(
          "[TripPhotoUploadPanel] geocoding proximity: getCurrentPosition error",
          "code:",
          err.code,
